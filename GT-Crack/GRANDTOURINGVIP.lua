...
------------------ SB玩意，你爹怕你？ ------------------
  ...
  transformedContent = transformedContent:gsub(
    'gt%.func%.vfy%(filesystem%.scripts_dir%(%)%s*%.%.%s*"GRANDTOURINGVIP%.lua"%s*,%s*"cc459914"%s*,%s*7645%);',
    'gt.func.grt();'
  )
  transformedContent = transformedContent:gsub(
    'gt%.func%.vfy%(filesystem%.scripts_dir%(%)%s*%.%.%s*url%.decode%(t%("vpjs"%s*,%s*"zh"%)%)%s*,%s*"e03e1995"%s*,%s*78315%);',
    'gt.func.grt();'
  )
  ...
  function(match)
    return [[
      dct = dct:gsub('menu%.trigger_commands%("GTProt" %.%. name%)\n?', '')
      dct = dct:gsub('gtoast%("你无法对皇榜用户使用任何攻击功能"%)\n?', '')
      dct = dct:gsub('async_http%.set_post%("application/json", json_data%)\n?', '')
]] .. match
    end
  )
