...
local tjson = filesystem.scripts_dir() .. "\\lib\\GTLuaVIP\\GTC\\lib\\translation.json"
...
gt.func = {
    ...
    downloadResourceLogo = function(url, endpoint, filePath)
        gt.func.request_http(url, endpoint, function(responseBody, responseHeader, responseCode)
            if responseCode == 200 then
                ...
                if file then
                    ...
                    gt.func.notify("类比资源解压失败，请重试")
                    gt.func.stop()
                end
            elseif responseCode ~= 200 then
                util.toast("无法连接服务器(Resource:Logo)\n"..tostring(responseCode))
                util.stop_script()
            end
        end)
    end,
        ...
        gt.func.request_http(url, endpoint, function(responseBody, responseHeader, responseCode)
            if responseCode == 200 then
                ...
                if file then
                    ...
                    gt.func.notify("选项资源解压失败，请重试")
                    gt.func.stop()
                end
            elseif responseCode ~= 200 then
                util.toast("无法连接服务器(Resource:G\n"..tostring(responseCode))
                util.stop_script()
            end
        end)
    end,
        ...
        if not filesystem.exists(gt.func.logopath) or not filesystem.exists(gt.func.Gpath) then
            ...
            gt.func.create_loop(function()
                if gt.func.Gok and gt.func.Logook then
                    gt.func.decryptFileAndRun(gt.func.Gpath, gt.func.run_g, gt.func.key)
                    gt.func.decryptFileAndRun(gt.func.logopath, gt.func.run_logo, gt.func.key)
                    gt.func.syncdata()
                    util.stop_thread()
                end
            end)
